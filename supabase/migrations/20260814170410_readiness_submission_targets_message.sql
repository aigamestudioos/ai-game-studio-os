-- Sprint 2.12c — achado do 2.12b (DECISIONS.md): a primeira Submission de
-- uma Release sempre nasce com o blocker SUBMISSION_TARGETS_MISSING antes de
-- ela existir — comportamento CORRETO (uma Release sem nenhuma Submission
-- ativa genuinamente não tem "plataforma-alvo" definida ainda), mas a
-- mensagem original ("A Release não tem nenhuma plataforma-alvo (Submission)
-- definida.") não deixava claro que isso é esperado e temporário, podendo
-- ser lido como um erro. Ajuste é só de TEXTO — a lógica (`not exists (...)`)
-- do 2.12a não foi tocada, conforme decisão registrada em DECISIONS.md de
-- não reabrir esse RPC sem autorização/defeito real.
...
begin
  ...
  if not exists (
    ...
    v_checks := v_checks || readiness_check_entry(
      ...
      'A Release ainda não tem nenhuma Submission para esta plataforma — normal antes de criar a primeira; deixa de bloquear assim que a Submission for criada e os demais checks passarem.',
      'RELEASE', p_release_id);
  end if;
