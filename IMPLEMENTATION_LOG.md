...
## Production Validation / Final Closure — Sprint 2.11d-2 (2026-08-14)

Retomado para fechamento em produção dos commits `1d9fc97` (2.11d-2c Google), `c3336ae` (2.11d-2d Apple) e `31c543f` (2.11d-2e validação), com autorização explícita do usuário para push, deploy, migrations e scheduler em produção.

**Fase 0 — memória com fixtures reais 10/50/100/200MB** (a bateria anterior só cobriu 96MB): benchmark reescrito para refletir o padrão exato do worker (1 buffer de 8MB residente por vez, nunca o artifact inteiro), medido isolado por processo (`node --expose-gc`). Resultado: delta de RSS 10.3MB (10MB)/24.7MB (50MB)/24.9MB (100MB)/38.3MB (200MB) — cresce sub-linearmente (20x o tamanho do artifact → só ~3.7x o delta), consistente com overhead de arena do V8 entre alocações de chunk, não com o artifact inteiro residente. Critério de "bounded" atingido — build/lint/typecheck (`turbo run build lint typecheck`) verdes, sem segredo versionado nos arquivos novos (grep negativo). Push liberado.

**Fase 1 — Push/Deploy**: `git push origin main` (`c3320a1..31c543f`) concluído. Deployment Vercel `dpl_CyN9Yb6GqPF5SvHsjqLHCwzpr2UV` (commit `31c543f`) confirmado `READY`/`production` via `list_deployments`.

**Fase 2 — Migrations**: as 8 migrations pendentes do 2.11d-2 (`null_safe_studio_checks`, `integration_jobs`, `job_claim_and_enqueue`, `resumable_session_vault`, `dispatcher_job_lifecycle_rpcs`, `pg_cron_dispatcher_schedule`, `grant_service_role_privileges`, `apple_operations_vault`) aplicadas via `apply_migration` (MCP) em produção. Mesmo achado do Sprint 2.11c: o MCP gera seu próprio timestamp de versão (`202608141351xx`) em vez de usar o prefixo do arquivo local — arquivos locais renomeados para bater exatamente com `supabase_migrations.schema_migrations`, mesma correção já aplicada em `c3320a1`. `pnpm check:schema` não pôde confirmar automaticamente (sandbox sem `SUPABASE_ACCESS_TOKEN`/`SUPABASE_DB_URL`) — sincronia confirmada manualmente comparando `list_migrations` (remoto) com `ls supabase/migrations/` (local) após o rename: 32/32 versões idênticas.

**Fase 3 em diante — BLOQUEIO GENUÍNO**: `cron.job` confirma `jobs-dispatcher-tick` agendado e `active=true` (aplicado pela própria migration), mas `job_dispatcher_config` está vazio em produção — `invoke_jobs_dispatcher()` é no-op silencioso até ser configurado com a URL real do dispatcher e um secret que precisa ser **idêntico** ao `JOBS_DISPATCHER_SECRET` já configurado como env var na Vercel (confirmado presente só localmente em `.env.local`, nunca lido daqui por ser segredo). Não há ferramenta MCP disponível nesta sessão para ler ou escrever env vars da Vercel — só para inspecionar deployments/logs. Configurar `job_dispatcher_config` com um secret inventado quebraria a autenticação real (o dispatcher rejeitaria) sem ganho nenhum; ler o secret real não é uma ação segura mesmo se houvesse ferramenta. Isso bloqueia: Fase 3 (scheduler real fim-a-fim), e por dependência, Fases 5/6/8/9/10 (que assumem o scheduler rodando em produção). Fase 4 (segurança do dispatcher) foi parcialmente exercitável — "sem secret"/"secret inválido"/"método incorreto" são testáveis sem o segredo real; "secret correto" não. Fase 7 (Playwright) tem bloqueio independente e já esperado (login/MFA humano).

**Classificação (naquele momento): PARCIAL.** Concluído com evidência real: memória bounded 10-200MB, build/lint/typecheck verdes, push, deploy Vercel READY, 8 migrations aplicadas e ledger local↔remoto sincronizado (32/32), `pg_cron` agendado e ativo. Não concluído por bloqueio genuíno de credencial (não fabricável, não substituível): configuração do secret do dispatcher em produção, e por consequência toda a validação empírica downstream do scheduler real (Fases 3, 5, 6, 8, 9, 10) e a bateria completa de Playwright (Fase 7, bloqueio independente por login humano). Próximo passo dependia do usuário: fornecer o valor de `JOBS_DISPATCHER_SECRET` já configurado na Vercel (ou gerar um novo e atualizá-lo nos dois lados) para que `set_job_dispatcher_config()` fosse chamado e o scheduler real pudesse ser validado ponta a ponta.

### Continuação — Fases 3-12 (2026-08-14, mesma data, sessão de fechamento)

O usuário gerou um novo `JOBS_DISPATCHER_SECRET`, atualizou a env var na Vercel Production e forneceu o valor no chat. Uma tentativa anterior desta continuação foi interrompida pelo harness (não por erro) antes de terminar. **Passo 0 — auditoria de estado**: antes de repetir qualquer ação, `job_dispatcher_config` foi consultado diretamente em produção e já estava preenchido (`tick_url = https://ai-game-studio-os-web.vercel.app/api/jobs/tick`, `secret_ref` presente), e `cron.job_run_details`/`net._http_response` já mostravam dezenas de ticks `succeeded`/`200` consecutivos a cada minuto desde `14:20:51`. Ou seja: a tentativa interrompida já havia chamado `set_job_dispatcher_config()` com sucesso antes de ser cortada — nenhuma reconfiguração foi necessária, só validação do que já estava rodando.

**Fase 3 — Scheduler real em produção: PASS.** `cron.job` (`jobs-dispatcher-tick`, `* * * * *`) ativo; cada tick chama `invoke_jobs_dispatcher()` → `net.http_post` → `/api/jobs/tick`; `net._http_response` confirma `status_code=200` de forma contínua (>20 execuções consecutivas antes mesmo de qualquer intervenção desta sessão). Também constatado, examinando `integration_jobs` remanescente da tentativa anterior (studio QA `262714a8-…`, provider `google_play`): um job real `SUCCEEDED` (checkpoint `{"done":true}`) e dois `FAILED` não-retryable (`MISSING_PACKAGE_NAME`) — prova de que claim→attempt→persistência de estado terminal já havia ocorrido ao vivo contra o dispatcher de produção.

**Fase 4 — Segurança do dispatcher: PASS (completo).** Via `curl` contra o endpoint de produção: sem header de secret → `401 {"error":"unauthorized"}`; secret inválido → `401`; método `GET` → `405`; `content-type` incorreto → `415 {"error":"unsupported_content_type"}`; corpo >1024 bytes → `413 {"error":"payload_too_large"}`. "Secret correto → 200" já demonstrado de forma mais robusta pelos ticks reais do `pg_cron` (dezenas de invocações bem-sucedidas) do que um `curl` manual isolado teria provado — e evita manusear o secret fora do fluxo do Vault. Nenhuma resposta, em nenhum caso, ecoa o secret recebido; `job_dispatcher_config.secret_ref` só guarda um ponteiro (`uuid`) para `vault.decrypted_secrets`, nunca o valor em texto puro; `studio_events`/`integration_jobs` não têm nenhuma coluna capaz de reter o secret do dispatcher (só dados de domínio do job).

**Fase 5 — Golden path assíncrono Google (produção): PASS, classificação TRANSPORTE VALIDADO / FUNCIONAL PENDENTE** (conforme esperado sem credencial Google real). Evidência real herdada da tentativa anterior + confirmada nesta sessão: `provider_upload` QUEUED → `integration_job` (`google_play`/`UPLOAD_BUILD`) → scheduler → dispatcher → claim real → tentativa real de transporte → estado terminal persistido (`SUCCEEDED` com checkpoint, e `FAILED`/`NON_RETRYABLE` com `MISSING_PACKAGE_NAME` — falha de configuração de credencial/produto, não de mecanismo). O mecanismo de fila/claim/dispatch está provado; o resultado funcional (upload de verdade na Play Console) depende de credencial real, fora do escopo desta sessão.

**Fase 6 — Golden path assíncrono Apple (produção): PASS, mesma classificação.** Nesta sessão, criado 1 `provider_upload` + `integration_job` (`apple_app_store`/`UPLOAD_BUILD`) isolado (studio QA `262714a8-…`, reaproveitando `store_connection` App Store já existente). O tick seguinte do `pg_cron` reivindicou, tentou e terminalizou o job como `FAILED`/`NON_RETRYABLE` (`MISSING_BUNDLE_IDENTIFIER`) — mesmo padrão do Google: transporte real (enqueue→scheduler→dispatcher→claim→tentativa→persistência) comprovado; falha é de configuração/credencial Apple ausente, esperada e não um defeito de mecanismo.

**Fase 8 — Concorrência em produção: PASS.** 6 jobs de teste (`integration_name='test'`, processor `test_echo`, sem efeito em provider real) enfileirados e 3 chamadas de `claim_integration_jobs()` disparadas simulando 3 dispatchers concorrentes (`qa-worker-A`, `qa-worker-B`, `qa-worker-C`): worker A reivindicou 3, worker B reivindicou os outros 3, worker C reivindicou 0 (fila já vazia) — `SKIP LOCKED` confirmado, zero double-claim (verificado por `GROUP BY claimed_by`: 3+3, nenhuma sobreposição).

**Fase 9 — Crash/recovery em produção: PASS.** Job 1: `start_integration_job_running` → `checkpoint_and_release_integration_job` com checkpoint parcial (`{"progress":42}`) → job volta a `QUEUED` com checkpoint persistido (continuação bounded real, GATE 9). Jobs 2 e 4: `lease_expires_at` forçado para o passado (simulando worker morto em `CLAIMED`/`RUNNING`) → `requeue_stale_jobs()` chamado (nova invocation simulada) → ambos recuperados para `QUEUED`, `attempt` incrementado, `last_error_code='WORKER_LEASE_EXPIRED'` — nenhum job preso em `RUNNING` eternamente. Jobs ainda dentro do lease (3, 5, 6) permaneceram `CLAIMED` corretamente (não foram afetados pela recuperação).

**Fase 10 — Scheduler failure: PASS.** `cron.unschedule('jobs-dispatcher-tick')` executado; confirmado `cron.job` com 0 linhas para o jobname. Um job de teste isolado (`QA_SCHEDULER_FAILURE`) enfileirado nesse período permaneceu `QUEUED` (não processado) enquanto o cron esteve desativado — método de detecção documentado: comparar `max(cron.job_run_details.start_time)` (último tick) / `max(net._http_response.created)` (última invocação real do dispatcher) contra `min(integration_jobs.scheduled_at) where status='QUEUED'` (job mais antigo parado na fila); um gap crescente entre esses timestamps é o sinal de scheduler morto. Cron reagendado (`cron.schedule`, novo `jobid=2`, mesma expressão `* * * * *`); ticks retomaram normalmente no minuto seguinte (`runid 77` em diante, todos `succeeded`) e o job que ficara parado (`QA_SCHEDULER_FAILURE`) foi reivindicado e terminalizado (`SUCCEEDED`) assim que o scheduler voltou — retomada automática confirmada, sem intervenção manual além de reativar o cron.

**Fase 11 — Cleanup QA: feito, parcial por design.** Removidos (dados mutáveis desta sessão): os 6 `integration_jobs` de concorrência, o job `QA_SCHEDULER_FAILURE`, o `integration_job`+`provider_upload` de teste da Apple. Resíduo intencional, não removido: os 3 `integration_jobs`/`provider_uploads` Google (`SUCCEEDED`+2×`FAILED`) herdados da tentativa anterior — mantidos como evidência já referenciada nesta entrada do log, mutação desnecessária; e todos os registros em `studio_events` (append-only por design, nunca alterado). Nenhuma tentativa de "zero resíduo" artificial.

**Fase 12 — Reclassificação final do Sprint 2.11d-2.**

### Classificação final: **PASS**

Justificativa: todos os elementos que antes bloqueavam PASS (secret do dispatcher, scheduler real, dispatcher de produção, concorrência, crash/recovery, scheduler failure/recovery, golden path assíncrono Google e Apple) agora têm evidência empírica direta contra produção real — não simulada, não inferida por leitura de código. O único item não coberto é a Fase 7 (bateria Playwright fim-a-fim via browser com login humano/MFA) — bloqueio independente, já esperado desde antes desta sessão, não relacionado ao secret do dispatcher nem a nenhum mecanismo testado aqui. Decisão de julgamento: esse gap fica documentado como pendência separada (infraestrutura de browser com autenticação humana não montada neste ambiente) e não impede o PASS geral, porque toda a superfície backend/scheduler/dispatcher/segurança/concorrência/recovery — que é onde o risco real do sprint estava concentrado (transferência assíncrona confiável para Google/Apple) — está comprovada ao vivo em produção.

**Bloqueios remanescentes:** nenhum bloqueio de credencial ou infraestrutura própria do sprint. Pendência documentada e separada: Fase 7 (Playwright E2E com login humano) — requer sessão com browser autenticado, fora do escopo de autonomia de uma sessão sem interação humana.
...
> **Atualização (2026-08-15) — Sprint 2.12 reclassificado de PARCIAL para CLOSED.** Fechado através do Sprint 2.13 — ver seção "Sprint 2.13" abaixo. As quatro pendências listadas acima foram endereçadas com evidência real: E2E via Playwright implementado e passando (GATE 3 do 2.13); Duplicate Submission prevenido por índice único parcial, validado com concorrência real (GATE 1); permissão granular de Publishing modelada e implementada (`publishing.read`/`publishing.create_submission`, GATE 2 — ao contrário do que o prompt do 2.13 antecipava como possível, este GATE não precisou ser adiado: o precedente de `studio.manage_store_connections` já existia e o trabalho era mecânico, não uma decisão arquitetural em aberto); Production Boundary Violation do 2.12c permanece um incidente de processo já auditado e documentado (não "corrigível" — é histórico), sem repetição neste sprint (autoridade de produção respeitada, sprint inteiro local-only). Esta nota preserva o registro PARCIAL original acima sem apagá-lo — a classificação PARCIAL descreve corretamente o estado do repositório entre 2026-08-14 e 2026-08-15.

## Sprint 2.13 — Publishing Hardening & E2E Foundation (2026-08-15)

**Objetivo:** fechar as três dívidas controláveis deixadas PARCIAL pelo Sprint 2.12: (1) ausência de E2E via navegador real, (2) unicidade de Submission não modelada, (3) autorização granular de Publishing inexistente. Local only — nenhuma escrita em produção em nenhum momento (regra de autoridade de produção explícita no prompt deste sprint).

**GATE 0 — Auditoria.** Confirmado: `submissions` tinha `idx_submissions_studio_status`/`idx_submissions_release_id`, nenhum `unique`. `permissions`/`role_permissions`/`current_user_has_permission()` já existiam desde o Sprint 1.8d-4, com precedente direto de permissão granular abaixo de "member" em `studio.manage_store_connections` (Sprint 2.12, `store_connections`) — refutando a suposição inicial de que GATE 2 precisaria ser adiado por falta de padrão a seguir. `turbo.json` já declarava `test:e2e` sem nenhum package implementá-la. Baseline antes de qualquer mudança: SQL 42/42, golden path HTTP 26/26, Vitest 8/8 — todos verdes.

**GATE 1 — Duplicate Submission (fechado).** Índice único parcial `idx_submissions_release_platform_active` em `(release_id, platform_id) WHERE archived_at IS NULL AND status NOT IN ('PUBLISHED','REJECTED','CANCELLED')` (`supabase/migrations/20260815000001_submission_duplicate_prevention.sql`). Decisão de granularidade e de regra de resubmissão documentadas em `DECISIONS.md`. Testado com concorrência real: `Promise.all` de duas criações simultâneas do mesmo Release+Platform resolve deterministicamente para exatamente 1 sucesso + 1 unique violation (não outro erro); resubmissão após a Submission anterior virar `REJECTED` é aceita. `scripts/test-readiness-golden-path.mjs` GATE 14 reescrito para essas 3 novas asserções (era 1 "ACHADO" documentando o bug; agora 3 asserções confirmando o fix).

**GATE 2 — Autorização granular de Publishing (fechado, não adiado).** `publishing.read` (ler `submissions`/readiness) e `publishing.create_submission` (criar Submission) — mesmo padrão de `studio.manage_store_connections`: catálogo + RLS split por comando + `get_release_readiness` com defesa em profundidade + backfill + `bootstrap_studio_for_current_user()` atualizado (`supabase/migrations/20260815000002_publishing_granular_permissions.sql`). Owner/Admin ganham as duas; Member só `publishing.read` — mudança de comportamento real e deliberada (Member deixa de poder criar Submissions por padrão). Matriz testada: `supabase/tests/readiness_test.sql` (fixture ganhou `roles`/`role_permissions` explícitos, 42/42 verde) e `scripts/test-readiness-golden-path.mjs` (Admin A/Member A com as permissões novas, 29/29 verde).

**GATE 3 — E2E Foundation (implementado, com limitação honesta documentada).** `@playwright/test` em `apps/web/e2e`. `playwright.config.ts` sobe `next start` (build de produção), não `next dev` — achado empírico deste GATE: `next dev` servia um chunk incompleto na primeira visita de cada rota sob a carga do Playwright, quebrando o mount do React com um parse error silencioso (sem nada visível na tela); `next start` eliminou o problema de forma determinística em execuções repetidas. `e2e/fixtures/seed.mjs` cria um Studio/Owner reais via Admin API (mesma técnica do golden path do 2.12c) e um Release com 2 Builds (Google Play + App Store); `e2e/critical-path.spec.ts` faz login real pela UI (nunca bypass de sessão), navega até Publishing, abre "New Submission", vê o painel de Readiness em NOT_READY com o blocker `STORE_CONNECTION_MISSING` visível, corrige via fixture direta no banco (documentado: não há UI real para conectar uma Store Connection sem credencial de verdade), clica "Recarregar" — conectando pela primeira vez a prop `onReload` do `ReadinessPanel`, que já existia desde o 2.12b mas nunca tinha sido usada por `apps/web/app/publishing/page.tsx` — vê READY, habilita "Criar Submissão", cria a Submission (App Store, 2ª do Release — a 1ª é pré-criada por fixture por causa da aspereza `SUBMISSION_TARGETS_MISSING` do 2.12b, documentada como limitação não corrigida neste sprint), confirma que aparece na lista, dá reload da página inteira e confirma que persiste. Passou de forma estável em execuções repetidas.

**GATE 4 — Regressão e fechamento.** `npx turbo run build lint typecheck test test:e2e`: 38/38 tasks verdes. Testes pré-existentes continuam passando: SQL 42/42 (+3 novas do GATE 1 = 29 no total do arquivo, ver detalhe acima), golden path HTTP 29/29 (26 originais + 3 novas do GATE 1), Vitest 8/8. `apps/web/vitest.config.ts` ganhou `exclude: ["e2e/**"]` (sem isso o Vitest tentava rodar os specs do Playwright e falhava). `turbo.json`: `test:e2e` ganhou `dependsOn: ["^build", "build"]` + `cache: false`.

**Arquivos alterados:** 7 modificados (`apps/web/app/publishing/page.tsx`, `apps/web/package.json`, `apps/web/vitest.config.ts`, `pnpm-lock.yaml`, `scripts/test-readiness-golden-path.mjs`, `supabase/tests/readiness_test.sql`, `turbo.json`) + 6 novos (`supabase/migrations/20260815000001_submission_duplicate_prevention.sql`, `supabase/migrations/20260815000002_publishing_granular_permissions.sql`, `apps/web/playwright.config.ts`, `apps/web/e2e/critical-path.spec.ts`, `apps/web/e2e/fixtures/seed.mjs`, `apps/web/.gitignore`) = 13 arquivos, 1 package tocado (`apps/web`) + `supabase/` (migrations/tests, não um package de código) + `scripts/`/`turbo.json` (infra de root). Bem dentro do limite de `CLAUDE.md` (~30 ideal/50 duro, 10 novos, 3 packages).

**Migrations aplicadas:** só local (Docker), via `npx supabase db reset` + `npx supabase migration up` implícito no reset. Nenhuma escrita em produção — nenhuma ferramenta MCP de escrita remota foi usada neste sprint.

### Classificação do Sprint 2.13: **PASS**

Todos os GATEs fecharam com evidência real, nenhum precisou da válvula de escape de adiamento prevista no prompt (GATE 2 foi implementado, não adiado — o precedente arquitetural já existia). Única ressalva honesta: GATE 3 não testa o fluxo de criar a PRIMEIRA Submission de um Release pela UI (bloqueado pela aspereza pré-existente do 2.12b, fora do escopo deste sprint corrigir) — documentado explicitamente no spec e em `DECISIONS.md`, não uma lacuna escondida.

## Sprint 2.14 — Submission Creation Completion (2026-08-17)

**Objetivo:** fechar o último rough edge conhecido do fluxo de Submission — `SUBMISSION_TARGETS_MISSING` impedindo a criação da 1ª Submission de um Release pela UI, débito documentado desde o Sprint 2.12b e contornado (não corrigido) por fixture no Sprint 2.13.

**Fase A — Investigação read-only.** Confirmado HEAD local `b3a2009` (não pushed), branch `main`, working tree limpo, baseline como documentado. `grep -rln "submission_target|SubmissionTarget"` em todo o repositório (código + SQL) — zero ocorrências fora do catálogo `readiness_check_definitions`: **não existe entidade `SubmissionTarget`**, é terminologia do catálogo 2.12a para a própria `Submission` na sua função de declarar plataforma-alvo. Lida a migration `20260814170230_readiness_check_definitions.sql` (define o `code`) e `20260814170321_get_release_readiness.sql`/`20260814170410_...` (avaliam o check): `SUBMISSION_TARGETS_MISSING` é `FAIL` sse a Release não tem nenhuma `Submission` ativa — condição de nível Release, avaliada uma única vez, fora do loop `for v_sub in (select ... from submissions ...)` que avalia todos os outros checks (`ARTIFACT_*`, `STORE_CONNECTION_*`, `PROVIDER_UPLOAD_*`, `BUILD_*`). Ou seja: para uma Release com zero Submissions, esse é estruturalmente o único check possível — não há como avaliar readiness de uma Submission que ainda não existe. Lido `submissions-repository.ts::create()` — `insert` direto contra `submissions` (RLS-protegido), sem nenhuma dependência do RPC de readiness; a criação em si nunca esteve gated no backend. Lido `apps/web/app/publishing/page.tsx` (estado pré-2.14): `isReady = readiness?.status === "READY"` — o único ponto do produto que tratava `SUBMISSION_TARGETS_MISSING` como impeditivo de criar.

**Causa raiz:** validação correta (o RPC está certo: uma Release sem Submission genuinamente não tem plataforma-alvo) disparada como blocker no momento errado — o Submission Gate (UI) interpretava um veredito que descreve exatamente a ação que o diálogo serve para completar como uma pré-condição a satisfazer antes dela. Não é bug de schema, não é lifecycle incompleto de uma entidade (não há entidade), não é dependência circular real — é a UI aplicando um check de nível Release-agregado como se fosse um check de nível "candidato a Submission".

**Stop Condition arquitetural avaliada e descartada:** nenhuma das 8 condições do prompt (redefinir ownership, alterar lifecycle de Release, nova etapa de produto, mudar Universal Language, quebrar contratos, migration destrutiva, arquitetura fora de Publishing, decisão de produto não respaldada) se aplica. A correção é inteiramente de interpretação client-side, sem tocar o RPC 2.12a/2.12c (mesma disciplina de não reabri-lo sem defeito real — aqui o defeito real não estava nele).

**Solução (GATE 6 do prompt — menor completa possível).** `apps/web/lib/readiness-status.ts` ganhou `isSubmissionGateBlocking(check)` e `isReadyForSubmissionGate(readiness)` — exclusão de um único `code` (`SUBMISSION_TARGETS_MISSING`) do cálculo de blockers do Gate, centralizado num único lugar e consumido por `apps/web/app/publishing/page.tsx` (botão "Criar Submissão") e `apps/web/components/publishing/readiness-panel.tsx` (badge de status e badges por check), para os dois nunca discordarem. O RPC continua devolvendo `blocking: true` honestamente (não foi alterado); a UI reinterpreta essa condição estruturalmente inevitável como informacional ("Resolvido ao criar a Submission") em vez de escondê-la. Nenhuma migration.

**GATE 1 — Primeira Submission real.** `apps/web/e2e/fixtures/seed.mjs` deixou de pré-criar a Submission Google Play direto no banco — o cenário nasce com Release READY (status `DRAFT`, dentro da lista submetível) e ZERO Submissions. `apps/web/e2e/critical-path.spec.ts` reescrito: login real → Publishing → "New Submission" → seleciona Release + Google Play → botão já habilitado (só `SUBMISSION_TARGETS_MISSING` FAIL, não-bloqueante para o Gate) → cria pela UI → confirma na lista → reload → persiste. Nenhum insert direto de Submission pelo teste, nenhuma chamada a RPC/SQL para produzir esse resultado.

**GATE 2 — Regressão de duplicidade (2.13).** Não tocado; `idx_submissions_release_platform_active` intacto. `scripts/test-readiness-golden-path.mjs` (29/29, incluindo as 3 asserções de concorrência/resubmissão do 2.13) e `supabase/tests/readiness_test.sql` (42/42) rodados sem alteração e passando — confirma zero regressão.

**GATE 3 — Autorização (2.13).** Não tocado; `publishing.read`/`publishing.create_submission` intactos — a correção do 2.14 é puramente de interpretação client-side de um veredito já lido sob a permissão existente, nunca um novo caminho de leitura/escrita.

**GATE 4 — E2E sem artifício.** Ver GATE 1 acima — a suíte deixou de depender de seed artificial de Submission. `playwright.config.ts` mantido em `next start` (nenhuma evidência nova justificou reabrir essa decisão do 2.13); comentário desatualizado do spec ("Docker + `next dev`") corrigido para refletir a config real.

**GATE 5 — Métricas.** `scripts/metrics.sh` corrigido: "Testes E2E" deixou de ser hardcoded `"0 (sem suíte configurada)"` (achado registrado no próprio METRICS.md do 2.13) e passou a contar `test(`/`test.only(` reais em `apps/web/e2e/*.spec.ts` + número de arquivos `.spec.ts`, dinamicamente — reflete "1 (em 1 arquivo(s) .spec.ts, apps/web/e2e — via Playwright)" na execução real deste sprint.

**GATE 6 — Regressão completa.** `pnpm run typecheck` ✅ (12/12 tasks), `pnpm run lint` ✅ (12/12 tasks), `pnpm run test` ✅ (Vitest 9/9 — 8 anteriores + 1 novo teste do Submission Gate), `pnpm --filter web build` ✅, `npx playwright test` (`apps/web`) ✅ 1/1, `bash scripts/test-readiness.sh` ✅ (SQL 42/42 + golden path HTTP 29/29). `./scripts/metrics.sh` executado e `METRICS.md` atualizado.

**Testes negativos cobertos:** Release NOT_READY continua bloqueando (teste Vitest pré-existente, inalterado); autorização insuficiente continua bloqueada em profundidade (RLS + RPC, 2.13, não tocados); duplicate ativo continua rejeitado (golden path HTTP); resubmissão pós-terminal continua aceita (golden path HTTP); isolamento entre Studios continua intacto (SQL + golden path, cross-Studio rejeitado); reload não altera estado persistido (E2E, passo final do teste).

### Product Delta

**ANTES da 2.14:** um usuário não conseguia partir de um Release READY sem Submission pré-existente e criar a primeira Submission pelo fluxo normal da aplicação — o botão "Criar Submissão" nunca habilitava para a 1ª Submission de um Release, mesmo com Build/artifact/Store Connection/metadata prontos, porque o Submission Gate tratava a ausência estrutural de Submissions como se fosse uma pré-condição não satisfeita. O E2E do 2.13 precisava fabricar essa Submission direto no banco para conseguir testar qualquer coisa além disso.

**DEPOIS da 2.14:** um usuário autorizado consegue partir de um Release READY sem Submission pré-existente e criar a primeira Submission pelo fluxo normal da aplicação, com persistência real e reload — sem nenhum artifício de teste ou de produto. O E2E prova isso pelo browser, do zero.

### Escopo

Arquivos modificados: `apps/web/app/publishing/page.tsx`, `apps/web/lib/readiness-status.ts`, `apps/web/components/publishing/readiness-panel.tsx`, `apps/web/app/publishing/page.test.tsx`, `apps/web/e2e/critical-path.spec.ts`, `apps/web/e2e/fixtures/seed.mjs`, `scripts/metrics.sh`, `DEFINITION_OF_DONE.md`, `DECISIONS.md`, `METRICS.md`, `IMPLEMENTATION_LOG.md`, `CHANGELOG.md` = 12 arquivos, 0 novos, 1 package tocado (`apps/web`) + `scripts/`/raiz (infra/docs). Bem dentro dos limites de `CLAUDE.md`. Nenhuma migration criada — MIGRATION LOCAL CRIADA: NÃO. MIGRATION REMOTA APLICADA: NÃO.

### Classificação do Sprint 2.14: **PASS**

Causa raiz identificada com evidência (não é entidade nova, é reinterpretação de UI de um check estruturalmente inevitável); solução mínima e local (sem migration, sem tocar RPC/RLS/permissão); GATE 1 (1ª Submission sem seed, pela UI, persistente) demonstrado pelo E2E reescrito; GATEs 2/3 (duplicidade/autorização do 2.13) revalidados sem regressão; GATE 5 (métricas) corrigido e validado; build/lint/typecheck/test/test:e2e/SQL/golden-path todos verdes; nenhuma operação de produção executada; nenhum push; commit local único ao final.

## Sprint 2.15 — Store Listing Management (2026-08-17)

**Objetivo:** fechar o bottleneck de produto identificado pelo Product & Architecture Gate: `METADATA_LISTING_MISSING` é um check `blocking=true` de Release Readiness cujo `source_of_truth` é `game_localizations`, mas essa tabela nunca teve repository/UI/Server Action usados pela aplicação — todo teste que chegava a READY criava `game_localizations` por fixture/service_role. Um usuário real não conseguia satisfazer esse requisito pelo produto.

**Fase 0/1 — Baseline e auditoria.** HEAD local confirmado em `7f5a438` (Sprint 2.14), branch `main`, `git status` limpo, `origin/main` atrás do local — baseline exatamente como esperado. Auditoria do schema: `game_localizations` (`supabase/migrations/20260716000005_games.sql`) tem `id, studio_id, game_id, language_code, title, short_description, full_description, keywords, metadata, created_at, updated_at`, com `unique(game_id, language_code)` — Store Listing é modelada por **Game + idioma**, não por plataforma. RLS já existia desde o Sprint 2.1 (`game_localizations_isolation`, mesmo padrão de isolamento por Studio de `games`/`builds`/`releases`/`game_versions` — sem GRANT nenhum para `anon`). `package_name`/`bundle_identifier` são colunas de `games` (não de `game_localizations`) e já eram editáveis via Server Actions (`setGamePackageName`/`setGameBundleIdentifier`, Sprint 2.11b/2.11c) dentro do painel "Enviar ao Google Play"/"Enviar à App Store" de um Build — nenhum caminho em Game. `get_release_readiness` (`20260814170321`) já lê `game_localizations` corretamente: `METADATA_LISTING_MISSING` = FAIL quando não existe nenhuma linha para o `game_id`; `METADATA_DESCRIPTION_MISSING` (WARN, não bloqueia) quando a linha existe mas `full_description` está vazio. Nenhum defeito real encontrado na RPC — não foi tocada.

**Permissão — investigado, não inventado.** Catálogo de permissions hoje: `studio.edit`/`studio.invite_members`/`studio.manage_members` (Sprint 1.8d-4), `studio.manage_store_connections` (2.8), `publishing.read`/`publishing.create_submission` (2.13). Não existe nenhuma permission `games.*`. A RLS de `games`/`game_versions`/`builds`/`releases`/`game_localizations` é hoje só isolamento por Studio (`USING (studio_id = current_user_studio_id())`, sem `current_user_has_permission()`) — qualquer member do Studio já pode editar essas tabelas hoje. Decisão: **não criar `games.manage_listing`** — reusar exatamente o mesmo modelo de autorização já vigente para o resto do domínio Game, por consistência (uma permission nova só para Store Listing, quando as tabelas irmãs não têm nenhuma, seria inconsistência arquitetural nova, não correção). Registrado como decisão de produto pequena e reversível, não como dúvida bloqueante.

**Solução.** `packages/database/src/repositories/game-localizations-repository.ts` (novo) — `listByGame`/`getPrimaryByGame`/`upsertPrimary`, MVP de um único idioma padrão (`en-US`, `STORE_LISTING_DEFAULT_LANGUAGE_CODE`) — o schema já comporta multi-idioma via `unique(game_id, language_code)`, mas a UI desta sprint não tem seletor de idioma (fora de escopo, registrado como dívida). `apps/web/hooks/use-game-listing.ts` (novo) — leitura client-side via repository (mesmo padrão de `use-game.ts`). `apps/web/app/games/[id]/listing-actions.ts` (novo, Server Action `saveStoreListing`) — escrita com validação de servidor, mesmo padrão das Server Actions de package_name/bundle_identifier já existentes; um único upsert cobre create e edit. `apps/web/components/games/store-listing-card.tsx` (novo) — card `Store Listing` (`id="store-listing"`) com formulário de ficha de loja (título/descrição curta/descrição completa/palavras-chave) e os dois campos de identificador (Package Name/Bundle Identifier), **reusando** `setGamePackageName`/`setGameBundleIdentifier` (2.11b/2.11c) em vez de duplicar a escrita — mesma fonte da verdade, sem quebrar o caminho existente do Provider Upload. Card inserido em `/games/[id]` (`apps/web/app/games/[id]/page.tsx`), ao lado do card "Plataformas" — sem página nova, sem navegação nova. `apps/web/lib/domain-events.ts` ganhou `StoreListingCreated`/`StoreListingUpdated` (mesmo padrão dos demais Domain Events do arquivo), emitidos pela Server Action.

**Deep link "Corrigir".** `apps/web/lib/readiness-status.ts::readinessFixHref` — `METADATA_LISTING_MISSING`/`METADATA_PACKAGE_NAME_MISSING`/`METADATA_BUNDLE_IDENTIFIER_MISSING` agora resolvem para `/games/{entityId}#store-listing` (o `entityId` desses três checks já era o `game_id`, entityType `GAME`, na RPC — não precisou de mudança no backend). Os demais checks (Store Connection, Artifact, Build) não foram alterados.

**RLS.** Nenhuma migration local foi necessária — a RLS de isolamento por Studio de `game_localizations` (2.1) já era suficiente para o modelo de autorização escolhido (sem permission granular nova). Escrito `supabase/tests/game_localizations_test.sql` (novo, mesma técnica de `readiness_test.sql`: tudo em `ROLLBACK`) cobrindo: Studio cria/edita sua própria Store Listing; Studio B (cross-Studio) não lê nem edita a Store Listing de Studio A; `anon` é bloqueado (permission denied — sem GRANT, nem chega a avaliar a policy); a condição usada pela RPC para `METADATA_LISTING_MISSING` (existir uma linha para `game_id`+`en-US`) fica satisfeita depois do insert. Achado honesto registrado no próprio teste: a policy de `game_localizations` (mesmo padrão de `builds`/`releases`/`game_versions`, todas desde o Sprint 2.1) valida `studio_id` mas não valida que `game_id` pertence a esse mesmo Studio — gap sistêmico pré-existente, não introduzido por esta sprint, e sem exposição real de produto porque a Server Action resolve `game.studio_id` via `getById()` já filtrado por RLS de `games` (um Game de outro Studio não é encontrado). Redesenhar RLS de todas as tabelas irmãs está fora do escopo do 2.15 — registrado como dívida.

**GATE 31 — critério decisivo.** `apps/web/e2e/fixtures/seed.mjs` deixou de inserir `game_localizations` (era a única linha do fixture que satisfazia `METADATA_LISTING_MISSING` artificialmente). `apps/web/e2e/critical-path.spec.ts` reescrito: logo após o login, o teste navega a `/games/{gameId}`, confirma o empty state ("Não cadastrada"), preenche e salva a Store Listing pela UI real, confirma "Cadastrada" e recarrega a página para provar persistência — só depois disso segue para `/publishing` e cria a 1ª/2ª Submission (fluxo herdado do 2.13/2.14, não reescrito). Sem essa etapa, `METADATA_LISTING_MISSING` (blocking, avaliado sempre, não só condicionado a Submission existir) bloquearia até a criação da 1ª Submission — diferente de `SUBMISSION_TARGETS_MISSING`, que o Submission Gate do 2.14 já ignora.

**Metrics.sh.** Investigado o achado repassado no briefing ("reporta 0 testes E2E hardcoded") — não se confirmou: `scripts/metrics.sh` já foi corrigido no Sprint 2.14 (GATE 5, ver entrada acima) e hoje deriva "Testes E2E" dinamicamente de `apps/web/e2e/*.spec.ts` (contagem real de `test(`/`test.only(`). Executado neste sprint, saída confere: "1 (em 1 arquivo(s) .spec.ts, apps/web/e2e — via Playwright)". Nenhuma correção adicional foi necessária.

**Documentação.** `PRODUCT_PROGRESS.md` — linhas 2.11c/2.11d, 2.13, 2.14 e 2.15 adicionadas (faltavam desde o achado do Product & Architecture Gate); `RELEASE_NOTES.md` — entradas 2.13/2.14/2.15 adicionadas; `PROJECT_STATUS.md` — só a linha "Implementação" da tabela de status atual foi atualizada (as tabelas sprint-a-sprint de Sprint 0/1 seguem desatualizadas, registrado como dívida, não reescritas por estarem fora do escopo mínimo pedido); `METRICS.md`/`IMPLEMENTATION_LOG.md` — este sprint.

**Testes.** Vitest 15/15 (9 pré-existentes + 6 novos — `readiness-panel.test.tsx` deep link + `store-listing-card.test.tsx` completo). SQL: `readiness_test.sql` 42/42 (revalidado, sem alteração) + `game_localizations_test.sql` novo, 8/8 asserções. Golden path HTTP 29/29 (revalidado). Playwright 1/1 (reescrito com o passo de Store Listing pela UI). `npx turbo run build lint typecheck test test:e2e` — 37/37 tasks + Playwright, todos verdes.

Arquivos modificados: `apps/web/app/games/[id]/page.tsx`, `apps/web/components/publishing/readiness-panel.test.tsx`, `apps/web/e2e/critical-path.spec.ts`, `apps/web/e2e/fixtures/seed.mjs`, `apps/web/lib/domain-events.ts`, `apps/web/lib/readiness-status.ts`, `packages/database/src/index.ts`, `packages/database/src/repositories/games-repository.ts`, `scripts/test-readiness.sh`, `PRODUCT_PROGRESS.md`, `PROJECT_STATUS.md`, `RELEASE_NOTES.md` = 12 arquivos. Novos: `apps/web/app/games/[id]/listing-actions.ts`, `apps/web/components/games/store-listing-card.tsx`, `apps/web/components/games/store-listing-card.test.tsx`, `apps/web/hooks/use-game-listing.ts`, `packages/database/src/repositories/game-localizations-repository.ts`, `supabase/tests/game_localizations_test.sql` = 6 novos. 2 packages tocados (`apps/web`, `packages/database`) + `supabase/tests`/`scripts`/raiz (infra/docs). Bem dentro dos limites de `CLAUDE.md`. Nenhuma migration criada — MIGRATION LOCAL CRIADA: NÃO. MIGRATION REMOTA APLICADA: NÃO.
...
**Contexto:** retomada do Sprint 2.16b sobre o commit `b56f2fa` (clientes HTTP reais Google/Apple + reconciliation + guard allowlist, já entregues numa sessão anterior). O escopo desta sessão era o item que faltava: fake provider server real, testes de integração contra Supabase local, testes de payload byte/JSON-exatos, expansão do Playwright, regressão completa e documentação.

**O que foi entregue nesta sessão:**
1. `packages/integrations/src/test-utils/fake-provider-server.ts` — fake provider HTTP **real**, um processo `node:http` escutando numa porta local de verdade (`server.listen(0, "127.0.0.1")`), não um mock de `fetch`/import in-process. Grava cada request recebida (método, URL, headers, body parseado) para asserção byte/JSON-exata, e permite 3 tipos de resposta programada por rota: `json` (status+body), `lost` (aceita a conexão e nunca responde — força o client a estourar seu próprio timeout via `AbortController`), `reset` (derruba o socket, erro de rede cru).
2. `packages/integrations/src/google-play/client.submission.test.ts` (10 testes) e `packages/integrations/src/apple/client.submission.test.ts` (10 testes) — contra o fake server real: `getGoogleEdit`/`getGoogleTrack`/`updateGoogleTrack`/`commitGoogleEdit` e `listAppleReviewSubmissions`/`createAppleReviewSubmission`/`createAppleReviewSubmissionItem`/`submitAppleReviewSubmission`/`getAppleReviewSubmission`. Cada teste verifica método HTTP, path, headers (`Authorization: Bearer ...`, `Content-Type`), e body serializado byte-a-byte (ex.: o body exato do `edits.tracks.update` do Google é `{ track, releases }`; o body exato do `createAppleReviewSubmission` é o envelope JSON:API `{ data: { type, attributes, relationships } }`, sem `itemType` no `reviewSubmissionItem`, confirmando a nota já deixada no código-fonte). Classes de erro cobertas: 401/403/409/429/500, e cenário de resposta perdida (o client falha limpo com `UNEXPECTED_ERROR`, nunca finge sucesso, nunca trava além do timeout de 10s). Chaves RSA/EC de teste são geradas em memória (`generateKeyPairSync`) — nunca uma credencial real.
3. `packages/integrations/package.json`/`vitest.config.ts` (novos) — o package `@agsos/integrations` não tinha script `test`/config de Vitest antes desta sessão (só `build`/`lint`/`typecheck`); os testes de client existentes (`classifyGoogleCommitError`) viviam em `apps/web` porque não havia onde rodar testes dentro do próprio package. Agora `@agsos/integrations` roda seus próprios testes (ambiente `node`, sem jsdom).

**Verificação de segurança:** grep por `sb_secret|service_role|private_key|BEGIN PRIVATE KEY|Authorization|Bearer|access_token|refresh_token` nos processors, no guard e nos novos arquivos de teste — os únicos matches são valores sintéticos de teste (chave RSA/EC gerada em memória, token fake `"fake-access-token"`), nunca uma credencial real, nunca persistidos fora da memória do processo de teste.

**Regressão completa desta sessão (números reais, não reaproveitados):** `npx turbo run build lint typecheck test` — 38/38 tasks verdes. `npx vitest run` em `apps/web` — 40/40. `npx vitest run` em `packages/integrations` — 20/20 (os 20 novos desta sessão). Total: 60 testes automatizados verdes.

**O que NÃO foi entregue nesta sessão (classificação honesta — ver reforço de integridade recebido a meio da sessão):**
- Testes de integração fim-a-fim contra a stack Supabase local real (GoTrue/PostgREST/Postgres) exercitando Submission → RPC → enqueue → dispatcher → processor → fake provider HTTP → checkpoint/reconciliation → estado terminal `SUBMITTED`, para Google e Apple.
- Cenários de idempotência empírica (duplo clique/submit concorrente, dois workers, lease expiration, dispatcher duplicate tick) provados executando o caminho real (não inferidos por leitura de código).
- Expansão do Playwright (`apps/web/e2e/`) para o lifecycle de Submission completo (sucesso, falha/retry, permissão de Member) via UI real.
- Por consequência, os itens do DoD "integration Google PASS", "integration Apple PASS", "idempotência confirmada empiricamente", "Playwright lifecycle PASS" continuam em aberto.

**Motivo:** o escopo completo (fake provider + integração Docker + Playwright + concorrência) excede o que esta sessão conseguiu entregar com o rigor exigido (nada de atalho/fixture fabricando o estado sob teste). Em vez de forçar uma versão superficial desses testes para "fechar" o sprint, a sessão preferiu entregar com solidez a camada que concluiu (fake provider real + payload exato) e reportar honestamente o que falta, conforme instruído explicitamente pelo prompt ("prefira classificar honestamente como PARCIAL... em vez de forçar").

**Classificação:** Sprint 2.16b permanece **PARCIAL** (2.16 consolidado também permanece PARCIAL — não reclassificado para PASS). Próximo passo explícito: sessão dedicada para os testes de integração Docker+dispatcher+fake-provider e a expansão do Playwright, antes de qualquer Production Validation Gate sobre este fluxo.

Arquivos desta sessão: `packages/integrations/src/test-utils/fake-provider-server.ts` (novo), `packages/integrations/src/google-play/client.submission.test.ts` (novo), `packages/integrations/src/apple/client.submission.test.ts` (novo), `packages/integrations/vitest.config.ts` (novo), `packages/integrations/package.json` (modificado — script `test` + devDependency `vitest`), `pnpm-lock.yaml` (modificado). Nenhuma migration, nenhum push, nenhuma chamada real a Apple/Google.

## Sprint 2.16c — Integrated Submission Lifecycle Proof (2026-08-18)

**Contexto:** o Sprint 2.16b entregou o fake provider server real e testes de payload byte-exato, mas isolados — não passavam pelo dispatcher nem tocavam o banco. Esta sessão fechou o gap: prova de ponta a ponta com Supabase local real + dispatcher real + fake provider real, para os fluxos de sucesso (Google, Apple) e um subconjunto priorizado de cenários de falha/resiliência, mais o Playwright do lifecycle.

**Auditoria rápida antes de codificar (seção 5 da spec).** Releitura de `transition_submission`/`complete_submission_job` (RPCs), `runDispatcherTick`/`processOneJob` (dispatcher), e dos dois processors de execução (`submission-google-play.ts`/`submission-apple.ts`) — confirmando o desenho documentado no 2.16a/b: state machine DRAFT→READY_TO_SUBMIT→SUBMITTING→SUBMITTED/FAILED, enqueue atômico em `transition_submission`, processor "continue" em cada etapa (edit→track→commit / listApps→reviewSubmission→item→submit), reconciliation via `edits.get`/`getReviewSubmission` antes de reenviar uma mutação ambígua.

**Fixture (`scripts/test-submission-integration.mts`, novo).** Mesmo padrão de `test-readiness-golden-path.mjs` (2.12c): `@supabase/supabase-js` com service role contra o stack local, `psql` só para o par de FK circular `studios`↔`users`. FIXTURE BOUNDARY documentado inline no topo do arquivo: legítimo fabricar auth/studio/roles/game/build/artifact/release/store_connection(+secret via RPC autenticado)/provider_upload SUCCEEDED (resultado do worker de TRANSPORTE, já testado em outro sprint); **nunca fabricado** — `submissions.status` além do INSERT inicial, `integration_jobs`/`submission_events`, ou qualquer efeito no fake provider, todos exclusivamente via `transition_submission`/`runDispatcherTick()`/processor real. `runDispatcherTick()` (mesma função que `/api/jobs/tick` chama) é importado e invocado diretamente em processo — evita depender de `next start` rodando só para os testes de integração de backend.

**Bugs reais encontrados e corrigidos (achados pelo próprio teste de integração, não hipotéticos):**
1. `adapter.listApps()` (Apple) e `adapter.createEdit()` (Google) nunca aceitavam um `baseUrl` — só os métodos de Review Submission/Track/Commit tinham esse parâmetro (adicionado no 2.16b). Como esses dois são a PRIMEIRA chamada de cada processor, mesmo com o guard `checkStoreMutationGuard` autorizado o processor ainda chamaria o host real do Google/Apple — inviabilizando qualquer teste de integração contra o fake provider, e sendo uma lacuna latente de correção mesmo fora de teste. Corrigido em `packages/integrations/src/{google-play,apple}/client.ts` (+ `adapter.ts`/`types.ts`), thread completo até os processors.
2. `UNEXPECTED_ERROR` (timeout, conexão perdida, exceção do client — nunca uma rejeição definitiva do provider) estava classificado `NON_RETRYABLE` em 4 pontos (commit final + `classifiedFailure()` nos dois processors). Isso quebrava a reconciliation do GATE 11/15: um "lost response" ia direto para `FAILED` terminal sem nunca reentrar no processor para consultar `edits.get`/`getReviewSubmission` e descobrir se a mutação de fato ocorreu — o comportamento documentado no próprio código-fonte ("nunca reenviar às cegas") não era o que o código fazia. Corrigido: `UNEXPECTED_ERROR` agora entra na lista retryable (mapeado para a mesma classe `RETRYABLE`/`INTERNAL`, ambas já retryable em `retry-policy.ts`), permitindo a reentrada e a reconciliation reais.

**Testes de integração reais (`scripts/test-submission-integration.mts`, 35/35 asserções):**
- Google sucesso ponta a ponta: RPC PREPARE/SUBMIT → 3 ticks do dispatcher real (createEdit→getTrack/updateTrack→commit) → `SUBMITTED`, checkpoint completo (`editId`/`track`/`versionCode`/`commitConfirmed`), exatamente 1 track mutation + 1 commit lógicos no fake provider, sequência de `submission_events` coerente.
- Google lost-response: fake provider aceita a conexão do commit e nunca responde (`kind:"lost"`) → job entra em `RETRY_WAIT` com `UNEXPECTED_ERROR` → reentra, reconcilia via `edits.get` (Edit ainda existe → commit anterior não ocorreu) → reenvia commit → `SUBMITTED`, exatamente 2 tentativas HTTP de commit (1 perdida + 1 bem-sucedida), nunca um commit cego duplicado.
- Google retry 500→200: primeira tentativa de commit 500, segunda 200 (com reconciliation `edits.get` entre as duas, mesma mecânica do lost-response) → `attempt` incrementa, job termina `SUCCEEDED`, exatamente 2 tentativas HTTP de commit.
- Google concorrência real: dois `runDispatcherTick()` disparados via `Promise.all` (não sequencial) competindo pelo mesmo job `QUEUED` → exatamente 1 dos 2 processa (claim via `FOR UPDATE SKIP LOCKED` do 2.11d), exatamente 1 `createEdit` lógico no fake provider.
- Apple sucesso ponta a ponta: RPC PREPARE/SUBMIT → dispatcher real (`listApps`→`listReviewSubmissions`/`createReviewSubmission`→`createReviewSubmissionItem`→`submit`) → `SUBMITTED`, checkpoint completo, exatamente 1 Review Submission + 1 Item + 1 submit lógicos.

**Não entregues nesta sessão (honestamente, dos 8 cenários de falha/resiliência da spec original):** worker crash recovery (reentrada após lease expirado no meio de uma etapa) e duplicate submit prevention como cenário dedicado (a unicidade lógica já é *coberta indiretamente* pelos cenários acima — nenhum deles duplicou mutação externa — mas não há um teste nomeado "dois SUBMIT simultâneos na mesma Submission"). Priorizados os 3 cenários julgados de maior valor (lost-response, retry, concorrência real do claim), conforme a própria instrução da sessão.

**Playwright (`apps/web/e2e/submission-lifecycle.spec.ts`, novo, 1/1):** estende o fluxo do `critical-path.spec.ts` (login real, Store Listing preenchida pela UI, Submission criada pela UI) até o lifecycle completo: botões reais "Preparar envio"/"Enviar" (`SubmissionLifecycleActions`, já existentes desde o 2.16a mas nunca exercitados por Playwright antes) → dispatcher real acionado via `POST /api/jobs/tick` (mesmo endpoint que `pg_cron` chamaria em produção) contra um `next start` dedicado (porta 3101, separada do `webServer` compartilhado de 3100) com `AGSOS_ALLOW_STORE_MUTATION=true` + allowlist apontando exclusivamente para um `FakeProviderServer` local efêmero → `SUBMITTED` → reload → persiste. Achado de infraestrutura: `spawn("npx", ["next", "start", ...])` deixa o processo real na porta vivo mesmo depois de `kill()` no wrapper `npx`; corrigido invocando o binário `next` diretamente (`node_modules/.bin/next`).

**Regressão completa desta sessão:** `pnpm --filter @agsos/integrations test` (20/20), `pnpm test` em `apps/web` (40/40), `pnpm typecheck` nos dois packages tocados (limpo), `bash scripts/test-submission-lifecycle.sh` (14/14 SQL), `node scripts/test-readiness-golden-path.mjs` (29/29), `npx playwright test` — `critical-path.spec.ts` + `submission-lifecycle.spec.ts` (2/2).

**Classificação do Sprint 2.16c:** **PARCIAL** — o núcleo pedido (fixture real com boundary explícito, testes de integração Google e Apple de sucesso ponta a ponta contra Supabase local + dispatcher real + fake provider real, 3 cenários de resiliência reais incluindo concorrência via `Promise.all`, Playwright do fluxo de sucesso completo, 2 bugs reais de produção encontrados e corrigidos) foi entregue com profundidade real, sem nenhum atalho fabricando o estado sob teste. Não entregue: worker crash recovery, teste dedicado de duplicate-submit, Playwright de falha/retry e do gate de permissão (Member sem `publishing.submit`).

**Classificação consolidada do Sprint 2.16 (2.16a+2.16b+2.16c):** **PARCIAL** — lifecycle completo, testado em unidade/SQL/HTTP/integração real/E2E para o caminho de sucesso e os cenários de resiliência mais críticos; PRODUCTION GUARD nunca foi desligado fora de teste local. Itens explicitamente em aberto para uma sessão futura, antes de qualquer Production Validation Gate sobre este fluxo: os 2 cenários de resiliência restantes (crash recovery, duplicate-submit dedicado) e a cobertura Playwright de falha/permissão.

Arquivos desta sessão: `scripts/test-submission-integration.mts` (novo), `apps/web/e2e/submission-lifecycle.spec.ts` (novo), `apps/web/lib/jobs/processors/submission-google-play.ts` (modificado — bugfix `UNEXPECTED_ERROR` + `guard.baseUrl` em `createEdit`), `apps/web/lib/jobs/processors/submission-apple.ts` (modificado — mesmo bugfix + `guard.baseUrl` em `listApps`), `packages/integrations/src/google-play/{client,adapter,types}.ts` (modificado — `baseUrl` em `createGoogleEdit`), `packages/integrations/src/apple/{client,adapter,types}.ts` (modificado — `baseUrl` em `fetchAppleApps`), `METRICS.md`, `IMPLEMENTATION_LOG.md`. Nenhuma migration, nenhum push, nenhuma chamada real a Apple/Google, nenhuma credencial real.

## Sprint 2.16d — Final Verification Closure (2026-08-18)

**Contexto:** fechamento dos 4 itens deixados explicitamente em aberto pelo 2.16c: worker crash-recovery, teste dedicado de duplicate-submit, Playwright de falha/retry e Playwright do gate de permissão Member. Sessão puramente de verificação + correção de bugs revelados — nenhuma feature nova, nenhuma migration.

**Baseline confirmado:** HEAD local em `82252fc` (Sprint 2.16c) batia exatamente com o esperado; nenhuma evolução posterior a reconciliar.

**GATE A/6 — worker crash recovery (`scripts/test-submission-integration.mts`, função `crashRecoveryTest`, mecanismo REAL — `claim_integration_jobs`/lease/`requeue_stale_jobs`, nunca simulado em memória):**
- Cenário 1 (crash genérico, antes de qualquer efeito externo): worker A reivindica o job via `claim_integration_jobs` (lease de 2s), "morre" sem nunca chamar o provider; o teste espera de verdade a lease expirar (2.2s reais, não avançada por SQL); `runDispatcherTick()` seguinte recupera via `requeue_stale_jobs` (attempt incrementa, `last_error_code = WORKER_LEASE_EXPIRED`), um novo worker processa do zero, exatamente 1 `createEdit` lógico no fake provider (o worker morto nunca chegou a chamar nada), converge para `SUBMITTED`.
- Cenário 2 (crash DEPOIS do provider aceitar a mutação — GATE 7, o caso mais importante): job avança por 2 ticks reais até checkpoint `{editId, trackUpdated:true}`; worker B reivindica (claim real, lease 2s); simula-se o crash chamando o MESMO client HTTP real do processor (`createGooglePlayPublishingAdapter(...).commitEdit(...)`) diretamente contra o fake provider — o commit é **de fato aceito pelo provider** — e então nada mais é persistido (worker B "morre" ali); a lease expira de verdade (2.2s); o novo worker reentra, e como o checkpoint já tinha `commitAttempted: true` persistido ANTES da chamada de rede (ver bugfix abaixo), cai na reconciliation existente (`edits.get`) → 404 (Edit já consumido) → converge para `SUBMITTED` sem nenhum segundo commit HTTP. Asserções: exatamente 1 commit lógico no total, exatamente 1 `edits.get` de reconciliação, `checkpoint.commitConfirmed === true` via reconciliação (nunca update manual).

**Bug real encontrado e corrigido (revelado pelo próprio Cenário 2 antes do fix, na fase de desenho do teste):** em `submission-google-play.ts` e `submission-apple.ts`, o checkpoint `commitAttempted`/`submitAttempted` só era persistido DEPOIS de `commitEdit`/`submitReviewSubmission` retornar (sucesso ou falha) — nunca antes de discar a chamada de rede. Um crash real do processo entre o provider aceitar a mutação e essa função retornar perdia essa marca por completo: o job reentraria pela via "sem commitAttempted", tentando um SEGUNDO commit/submit cego sobre uma entidade já consumida pelo provider, em vez de cair na reconciliation (`edits.get`/`getReviewSubmission`) já existente desde o 2.16b/c. Corrigido: ambos os processors agora escrevem `commitAttempted: true`/`submitAttempted: true` diretamente em `integration_jobs.checkpoint` (via `admin.from("integration_jobs").update(...)`) IMEDIATAMENTE antes da chamada de rede — não depois. Qualquer crash a partir desse ponto (antes, durante ou logo depois da chamada HTTP) agora sempre reentra pela reconciliation existente, nunca repete cegamente. Nenhuma mudança de state machine, nenhum novo branch de reconciliation — só a ordem de persistência do marcador que já existia.

**GATE B/8/9 — duplicate submit dedicado (mesmo script, função `duplicateSubmitTest`):**
- Cenário 1: duas chamadas `transition_submission(..., 'SUBMIT', ...)` disparadas via `Promise.all` de verdade (não sequencial) na mesma Submission `READY_TO_SUBMIT`. O `select ... for update` de `transition_submission` (já existente desde o 2.16a, GATE 13) serializa as duas chamadas via row lock: exatamente 1 faz a transição real (`noop:false`, cria o job), a outra retorna `noop:true` sem erro e sem criar nada. Asserções: exatamente 1 `integration_jobs`, exatamente 1 evento `SubmissionSubmissionStarted`, submission transiciona 1x para `SUBMITTING`. Reconfirma também que o dispatcher concorrente (`Promise.all([runDispatcherTick(), runDispatcherTick()])`) sobre o job resultante ainda processa 1x só.
- Cenário 2: job já `SUBMITTING` (QUEUED ativo) e o usuário chama `RETRY` de novo — `noop:true`, nenhum segundo job ativo criado.
- Nenhum bug encontrado neste gate — o mecanismo do 2.16a (row lock + índice de job ativo) já era correto; o gap era só a ausência de um teste NOMEADO provando isso isoladamente (antes só era coberto indiretamente).

**GATE C — Playwright failure/retry (`apps/web/e2e/submission-lifecycle-failure.spec.ts`, novo, porta dedicada 3102):** fluxo real via UI (login → Store Listing → criar Submission → Preparar → Enviar) até o fake provider responder 403/FORBIDDEN (não-retryable, escolhido deliberadamente para não depender do loop de retry automático 429/500, que já é coberto pelos testes de integração) no primeiro `createEdit` → `FAILED` visível na UI → assertions de sanitização (mensagem da UI não contém o erro cru do provider nem qualquer termo sensível — `private_key`/`access_token`/`bearer`/`service_account`) → botão "Retry" visível → muda o fake provider para sucesso → clica Retry pela UI → dispatcher real → `SUBMITTED` → reload persiste. Nenhum update direto de `submissions.status`/`integration_jobs` em nenhum momento.

**GATE D — Playwright Member permission (`apps/web/e2e/submission-lifecycle-member.spec.ts`, novo, porta dedicada 3103):**
- UI: Member (papel com só `publishing.read` + `publishing.create_submission`, sem `publishing.submit`) faz login real, abre uma Submission existente (criada pela sessão Owner, fixture legítima) — nenhum botão "Preparar envio"/"Enviar"/"Retry" aparece.
- Backend: sessão Member real (não `service_role`) chama `transition_submission(..., 'PREPARE', ...)` diretamente — RPC nega com `sem permissão publishing.submit`; `submissions.status` confirmado inalterado (`DRAFT`).

**Bug real encontrado e corrigido (UI, revelado ao desenhar este gate):** `SubmissionLifecycleActions` (desde o 2.16a) mostrava o botão de ação sempre que a state machine client-side permitia, **independente de permissão** — o servidor sempre rejeitava a chamada corretamente (`transition_submission` já checava `publishing.submit` desde o início), mas a UI sugeria incorretamente para qualquer papel (ex.: Member) que a ação estava disponível, violando o critério explícito da seção 12 da spec deste sub-sprint ("botão Submit inexistente ou desabilitado"). Corrigido com o menor fix possível, escopado só a este componente: um `useEffect` chama `current_user_has_permission('publishing.submit')` (RPC já existente, já `grant`ada a `authenticated` desde o Sprint 2.4) e o botão só é renderizado quando essa checagem confirma `true` — nunca assume permitido enquanto carrega (`default: false`). Não foi criado nenhum sistema de permissão novo, nenhuma mudança de RLS/RPC — só a UI parou de contradizer o que o backend já impunha.

**Process/port hygiene:** cada spec novo roda em porta dedicada (3102 falha/retry, 3103 Member — distintas de 3100 padrão/3101 sucesso), mesmo padrão de `spawn` do binário `next` direto (não `npx next`, achado de leak do 2.16c) + `kill("SIGKILL")` + `await exit` no `afterAll`. Confirmado via `ps aux`/`lsof` depois da suíte completa: nenhum processo `next start` nem porta 3100-3103 órfã.

**Security/secret leakage:** inspecionado manualmente — checkpoints de `integration_jobs` (Google/Apple) nunca contêm token/chave privada/JSON de service account, só IDs/flags técnicos (`editId`, `track`, `versionCode`, `commitAttempted`, `reviewSubmissionId`, etc., já assim desde o 2.16b); a nova escrita de checkpoint do bugfix (`commitAttempted`/`submitAttempted` cedo) usa exatamente os mesmos campos, nada novo exposto. Página FAILED do Playwright confirmada sem o erro cru do provider nem termos sensíveis (assertion automatizada, ver GATE C acima).

**Regressão completa desta sessão (números reais, não copiados de sessões anteriores):**
- `npx tsx scripts/test-submission-integration.mts`: **71/71** asserções (35 herdadas do 2.16c + 9 crash-recovery + 18 duplicate-submit + regressão implícita dos cenários existentes rodando com o processor corrigido).
- `bash scripts/test-submission-lifecycle.sh` (SQL): **20/20** ok, 0 falhas.
- `node scripts/test-readiness-golden-path.mjs` (HTTP golden path): **29/29**.
- `pnpm --filter @agsos/integrations test`: **20/20**.
- `pnpm test` em `apps/web`: **40/40**.
- `npx playwright test` (suíte completa, `apps/web/`): **5/5** — `critical-path.spec.ts` (1), `submission-lifecycle.spec.ts` (1, sucesso — regressão confirmada com o fix de permissão aplicado), `submission-lifecycle-failure.spec.ts` (1, novo), `submission-lifecycle-member.spec.ts` (2, novo).
- `npx turbo run build lint typecheck test`: **38/38 tasks successful** (build+lint+typecheck+test em todos os packages/apps do monorepo).

**Classificação do Sprint 2.16d:** **PASS** — os 4 gates pedidos foram fechados com profundidade real (mecanismo de crash/lease real, não simulado em memória; concorrência real via `Promise.all`; Playwright real via UI + dispatcher real, nunca update direto de estado terminal), 2 bugs reais adicionais foram encontrados e corrigidos (gap de reconciliation em ambos os processors; UI de permissão do Member), toda a regressão (SQL/HTTP/Vitest/integração/Playwright/turbo) roda limpa nos números acima.

**Classificação consolidada do Sprint 2.16 (2.16a+2.16b+2.16c+2.16d): PARCIAL → PASS.** Todos os 18 critérios da seção 19 da spec do 2.16d atendidos: worker crash-recovery, crash-after-effect com reconciliação, duplicate-submit dedicado, nenhum job ativo duplicado, Playwright de sucesso/falha-retry/Member (UI+backend), Google/Apple/lost-response/concorrência, zero vazamento de segredo, SQL/HTTP/Vitest/Playwright/turbo completos, documentação atualizada.

Arquivos desta sessão: `scripts/test-submission-integration.mts` (modificado — +2 gates), `apps/web/e2e/submission-lifecycle-failure.spec.ts` (novo), `apps/web/e2e/submission-lifecycle-member.spec.ts` (novo), `apps/web/lib/jobs/processors/submission-google-play.ts` (modificado — bugfix persistência antecipada de `commitAttempted`), `apps/web/lib/jobs/processors/submission-apple.ts` (modificado — mesmo bugfix para `submitAttempted`), `apps/web/components/publishing/submission-lifecycle-actions.tsx` (modificado — bugfix UI: botão de ação só aparece após confirmar `publishing.submit` via RPC), `METRICS.md`, `IMPLEMENTATION_LOG.md`, `DECISIONS.md`. Nenhuma migration, nenhum push, nenhuma chamada real a Apple/Google, nenhuma credencial real.

## Sprint 2.16 — Production Validation Gate — Phase A (2026-08-18)

**(1) Phase A initial attempt — FAILED.** A migration `20260817000001_submission_lifecycle.sql` foi aplicada em produção via MCP `apply_migration` numa sessão anterior; o mecanismo do MCP gerou um identity de ledger diferente (`20260818124539`) em vez de preservar `20260817000001`. `20260817000002_submission_lifecycle_rpcs.sql` nunca foi aplicada. Resultado: schema de produção ficou num estado intermediário — `submission_status` com os 3 valores novos (`READY_TO_SUBMIT`/`SUBMITTING`/`FAILED`), mas sem `transition_submission`, `complete_submission_job`, permissão `publishing.submit`, tabela `submission_events` nem `integration_jobs.submission_id`.

**(2) Tentativas de recovery bloqueadas por falta de credencial CLI** — sessões anteriores não tinham acesso a `SUPABASE_ACCESS_TOKEN` para autenticar o Supabase CLI oficial, então não foi possível reconciliar o ledger sem recorrer a SQL manual (proibido pela regra central do recovery) ou reaplicar via MCP (mesmo mecanismo que causou a divergência).

**(3) Phase A Recovery bem-sucedida (2026-08-18).** Token carregado de `.env.supabase-cli-token.local` (nunca impresso). `npx supabase projects list` autenticou com sucesso; projeto já estava linkado a `vkyswyuxitwakjqjteso`. `npx supabase migration list` confirmou exatamente a divergência esperada: `20260817000001`/`20260817000002` local-only, `20260818124539` remote-only.

Antes de qualquer reparo, migration 1 foi provada equivalente ao schema de produção via query read-only (`pg_enum`/`pg_type`): os 3 valores (`READY_TO_SUBMIT`, `SUBMITTING`, `FAILED`) — todo o efeito do arquivo — já estavam presentes.

Reparo via `npx supabase migration repair --linked`: `--status reverted 20260818124539` (remove a identity incorreta do ledger) seguido de `--status applied 20260817000001` (marca a migration real como aplicada). Nenhum SQL manual, nenhuma mutação de conteúdo de migration. `migration list` pós-reparo confirmou: `20260817000001` local=remote, `20260817000002` local-only/pending, `20260818124539` ausente — única migration pendente era exatamente a esperada.

`npx supabase db push --linked` aplicou somente `20260817000002_submission_lifecycle_rpcs.sql` (confirmado no output: `"migrations":["20260817000002_submission_lifecycle_rpcs.sql"]`, sem reaplicar migration 1, sem gerar novo timestamp). `migration list` final: ambas `applied`, local=remote, nenhum identity órfão.

Validação de schema pós-push (queries read-only): `publishing.submit` presente e concedida a Owner (40 grants) e Admin (20 grants) em todos os studios; `submission_events` existe, RLS habilitado, policy `submission_events_select` correta (`studio_id = current_user_studio_id() AND current_user_has_permission('publishing.read')`), sem policy de insert para `authenticated`/`anon`; `integration_jobs.submission_id` existe; constraint `integration_jobs_exactly_one_target` com definição idêntica ao arquivo da migration; `transition_submission(uuid,text,uuid)` e `complete_submission_job(uuid,uuid,text,text,integer)` existem, `SECURITY DEFINER`, grants corretos (`transition_submission` → `authenticated`+`service_role`, sem `anon`; `complete_submission_job` → só `service_role`); todos os 5 índices esperados presentes.

Data safety (read-only): `submission_events` com 0 linhas (migration não criou eventos artificiais); `submissions` com 0 linhas (nenhum dado pré-existente a corromper); `integration_jobs` com as 3 linhas históricas pré-existentes intactas (1 `SUCCEEDED` + 2 `FAILED`, todas com `submission_id IS NULL` — não tocadas pela nova coluna/constraint).

Application smoke via HTTP contra `aigamestudioos.com`: `/` → 200, `/login` → 200, `/dashboard`/`/games`/`/publishing` → 307 (redirect de auth, esperado, não-500), `/api/jobs/tick` → 415 (POST sem content-type)/405 (GET) — nenhum 500 em nenhuma rota. Nenhuma Submission real criada, dispatcher não executado contra job real.

Guard de mutação de loja revalidado por leitura de código (`apps/web/lib/jobs/store-mutation-guard.ts`): dupla camada fail-closed inalterada nesta sessão — nenhuma variável de ambiente Vercel foi lida ou alterada.

**Classificação: PRODUCTION_VALIDATED_PHASE_A.** Ledger reconciliado via CLI oficial, migration 2 aplicada, schema completo, zero SQL manual no ledger, zero mutação de loja, zero dado de produção alterado além do schema desta migration.
