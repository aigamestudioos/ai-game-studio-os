...
echo
echo "=== supabase/tests/game_localizations_test.sql (Sprint 2.15 — RLS de Store Listing, banco local, tudo em ROLLBACK) ==="
docker exec -i "$CONTAINER" psql -v ON_ERROR_STOP=1 -U postgres -d postgres \
  < supabase/tests/game_localizations_test.sql
