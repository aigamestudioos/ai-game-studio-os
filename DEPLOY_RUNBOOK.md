...
## 13. `studio_events` genérico absorve um evento novo sem nenhuma migration — mas a RLS de isolamento precisa ser revalidada mesmo assim (Sprint 2.10.1)

`StoreConnectionCallCompleted` (Integration Health) é só mais um `event_name` na tabela genérica `studio_events` (Sprint 1.7) — não precisou de coluna, tabela ou policy nova, já que a RLS `studio_events_isolation` já cobre `INSERT`/`SELECT`/`UPDATE`/`DELETE` para qualquer `event_name`. Mesmo assim, o isolamento entre Studios foi revalidado especificamente para este evento (não assumido por herança): criado um segundo Studio + usuário local, confirmado que uma tentativa de `INSERT` com `studio_id` de outro Studio é bloqueada (`403`), e que um usuário do segundo Studio lê `[]` ao consultar `StoreConnectionCallCompleted` enquanto o founder vê exatamente as suas próprias linhas. Mesma disciplina do §12 — "sem SQL novo" não é motivo para pular a revalidação de um caso de uso novo sobre uma policy existente.

**Nota de ambiente:** neste ciclo, `supabase start` completo (com `realtime`/`storage-api`/`vector`/etc.) falhou repetidamente por health-check timeout — ambiente com pouca memória livre disponível na sessão. Contornado com `supabase start -x realtime,storage-api,imgproxy,studio,edge-runtime,logflare,vector`, que sobe só Postgres/PostgREST/GoTrue — suficiente para todo teste deste sprint (nenhum depende de Realtime/Storage/Vector). Registrado como alternativa válida para sessões futuras com o mesmo sintoma, não como mudança permanente de processo.

## 14. `seed.sql` nunca roda em produção — nenhuma tabela "global read-only" está pronta sem uma migration própria (achado crítico, Fase 0 do Sprint 2.11)

Achado no primeiro smoke-check de produção via UI (não via RPC direta) da história do projeto: `platforms` estava vazia em produção desde a criação do schema (Sprint 1.7/2.8), porque suas linhas só existiam em `supabase/seed.sql`/`supabase/seed/01_global.sql` — e **o Supabase CLI nunca aplica `seed.sql` a um projeto hospedado/linkado**, só a `supabase db reset` local. `supabase db push` aplica exclusivamente `supabase/migrations/`. Nenhuma migration jamais fez `insert into platforms`, então a feature de Store Connections (Sprint 2.8/2.9/2.10/2.10.1) nunca foi utilizável em produção por nenhuma conta — o botão "Add Connection" ficava sempre desabilitado (sem nenhuma `platforms` row para referenciar).

**Por que a validação de produção do Sprint 2.9.1 não pegou isso:** aquela validação testou os `GRANT`s das RPCs de Vault diretamente via REST/RPC (que não dependem de nenhuma `platforms` row existir) — nunca o fluxo real de criar uma Store Connection pela UI, que é o único caminho que de fato depende de `platforms` ter dados.

**Correção:** `20260809000001_platforms_seed_backfill.sql` — `insert into platforms (...) on conflict (id) do nothing`, usando os MESMOS UUIDs fixos do seed local (não gerados agora), para não quebrar nenhuma referência hardcoded existente em código/testes. `seed.sql`/`seed/01_global.sql` precisaram do mesmo `on conflict (id) do nothing` no insert de `platforms` — sem isso, a migration (que roda antes do seed) e o seed local colidiam em `duplicate key`, quebrando `supabase db reset` local. Aplicada seguindo o Gate de Schema (§10) completo: testada com reset local limpo, `dry-run` revisado, aplicada em produção, confirmada com `pnpm check:schema` E uma chamada REST independente confirmando as 3 linhas.

**Regra nova:** nenhuma tabela "global, só leitura, populada via seed" (`platforms`, `countries`, `languages`, `currencies`, e qualquer futura) pode ser considerada pronta para uso em produção sem uma migration própria que a popule — `seed.sql` cobre só o ambiente local, nunca produção, para nenhuma tabela. **Débito técnico registrado, não corrigido nesta sessão:** `countries`/`languages`/`currencies` (mesmo `seed/01_global.sql`) provavelmente têm o mesmo problema em produção — só `platforms` foi confirmado e corrigido aqui, por ser o bloqueio direto do smoke-check.

## 15. Upload resumível (TUS) ao Supabase Storage não usa signed URL — precisa de RLS real em `storage.objects` (Sprint 2.11a)

Achado durante a implementação do upload direto do browser para o bucket `builds`: o plano inicial assumia que `createSignedUploadUrl` (token assinado por objeto) cobriria também o caminho resumível — não cobre. Esse método só existe para upload simples (um único `PUT`, sem retomada); o endpoint resumível (`/storage/v1/upload/resumable`, protocolo TUS) não emite nem aceita esse tipo de token. A única forma real de autorizar um upload resumível direto do browser é uma policy de RLS de verdade em `storage.objects`, avaliada contra a sessão do próprio usuário (`anon key` + JWT dele) — nunca a `service_role` (que nunca deve chegar ao browser).

**Correção:** policy `build_artifacts_object_insert` (`20260810000001_build_artifacts.sql`) restringe o `INSERT` em `storage.objects` ao bucket `builds`, ao primeiro segmento do path (`storage.foldername(name)[1]`) igual ao `studio_id` do usuário autenticado, e à permissão `builds.manage_artifacts`. Download e remoção continuam sem nenhuma policy para `authenticated` — só alcançáveis via `service_role` (signed URL / admin client), confirmado localmente: `authenticated` tentando baixar direto (sem signed URL) recebe `404`/"Object not found", nunca os bytes do objeto.

**Regra nova:** ao planejar upload resumível/TUS para qualquer bucket futuro, nunca assumir que signed-URL cobre esse caminho — verificar contra o comportamento real do endpoint resumível (ou a documentação específica de TUS do Supabase Storage, não a de upload simples) antes de desenhar a policy.

## 16. Cleanup de dados de teste QA em produção: `studios ↔ users` tem FK circular, e provar que `studio_events` é de teste exige checar `actor_id`/`event_name`, não só `studio_id` (Sprint 2.11a)

Achado ao remover Studios/Users descartáveis criados para testar o Sprint 2.11a contra produção real: `public.studios.owner_user_id` (`NOT NULL`, referencia `users.id`) e `public.users.studio_id` (`NOT NULL`, referencia `studios.id`) formam uma FK circular. Nenhuma ordem de `DELETE` via PostgREST remove o último par Studio+User de um lote — deletar `users` primeiro é bloqueado por `fk_studios_owner_user_id` (o studio ainda referencia o user como owner); deletar `studios` primeiro é bloqueado por `users_studio_id_fkey` (o user ainda referencia o studio). Sem acesso a `psql` direto em produção (só REST/RPC/Admin API e o Supabase CLI, que não expõe execução de SQL arbitrário), a única solução limpa é um script SQL que torna as duas constraints `DEFERRABLE` só dentro de uma transação, resolve os deletes, e reverte para `NOT DEFERRABLE` antes do commit — nunca alterar a constraint permanentemente.

**Segundo achado, numa segunda tentativa:** a primeira versão do script de cleanup quebrou com `23503` em `studio_events_studio_id_fkey` — os eventos emitidos pela suíte de testes (`BuildArtifactUploadStarted`, etc.) ainda referenciavam os Studios QA, e a v1 do script não previa apagar `studio_events` antes de `studios`. A correção óbvia (`DELETE FROM studio_events WHERE studio_id IN (...)`) foi deliberadamente rejeitada como insuficiente: `studio_id` sozinho não prova que um evento é de teste, só que ele pertence àquele tenant — um bug em outro lugar do sistema poderia, em tese, ter gravado um evento com `studio_id` errado. A correção usada exige duas provas independentes antes de qualquer `DELETE` em `studio_events`: todo `actor_id` dos eventos daqueles `studio_id` está dentro do conjunto exato de `user_id` QA da sessão (nunca nulo, nunca um usuário fora do lote), **e** todo `event_name` está dentro da lista fechada de eventos que aquele teste específico poderia ter emitido (`BuildArtifactUploadStarted`, `BuildArtifactStored`, etc. — nunca, por exemplo, um evento de outro domínio que não deveria existir para esses Studios). Qualquer violação aborta a transação inteira (`RAISE EXCEPTION`) antes de apagar qualquer linha.

**Regra nova:** ao limpar dados de teste em produção que envolvam entidades com FK circular (`studios`↔`users` neste schema), planejar a quebra controlada de constraint via `ALTER CONSTRAINT ... DEFERRABLE` dentro da própria transação de cleanup, revertendo antes do commit — nunca deixar uma constraint permanentemente deferível. E ao decidir remover qualquer linha "porque é de teste", `studio_id`/tenant sozinho nunca é prova suficiente quando existe um segundo campo (aqui, `actor_id`) capaz de confirmar a autoria — sempre cruzar os dois antes de um `DELETE` em produção, com guardrails que abortam (não que seguem em frente com aviso) se a prova falhar.

## 17. `ALTER CONSTRAINT ... NOT DEFERRABLE` num script de cleanup pode quebrar produção se o estado "original" nunca foi documentado (achado crítico, validação do Sprint 2.11b)

O script de cleanup do GATE 9 (Sprint 2.11a, §16 acima) tornava `fk_studios_owner_user_id`/`users_studio_id_fkey` deferíveis só durante a própria transação, e as revertia para `NOT DEFERRABLE` ao final, assumindo que esse fosse o estado original. Era o estado original de `users_studio_id_fkey` — mas **não** de `fk_studios_owner_user_id`, que precisa ser `DEFERRABLE INITIALLY DEFERRED` para `bootstrap_studio_for_current_user()` funcionar (insere em `studios` antes de `users`, na mesma transação). O "revert" quebrou silenciosamente todo onboarding de conta nova em produção, sem nenhum teste local pegar isso (não havia teste automatizado cobrindo signup real pós-cleanup).

**Regra nova:** antes de reverter qualquer `ALTER CONSTRAINT` num script de cleanup/hotfix, **verificar empiricamente** (não assumir) qual era o estado antes da alteração — rodar o fluxo de negócio real que depende daquela constraint (aqui, `bootstrap_studio_for_current_user()` com uma conta nova de verdade) tanto antes quanto depois de qualquer script que toque `ALTER CONSTRAINT`, mesmo quando o script parece "só" limpar dados de teste.

## 18. Hard-delete de Studio é incompatível com Event Store append-only — política de cleanup revisada (Sprint 2.11b)

Ver `DECISIONS.md` [2026-08-10] para o achado completo. Resumo operacional: ao limpar dados QA de uma validação de produção, **nunca** tentar `DELETE` em `studios`/`users` owner nem em `studio_events` — sempre bloqueado por design (Event Store append-only) ou por FK circular entre transações separadas. Cleanup correto: remover todo dado operacional mutável (builds, artifacts, connections, invites, roles, etc.) e banir o(s) `auth.users` via Admin API (`ban_duration` longo) em vez de apagar. Studios/Users QA residuais em produção são esperados e devem ser identificáveis pelo prefixo `qa-` no nome/email — nunca reportar isso como "zero resíduo".
